...
from driver.send_admin import send_admin
...
async def driver_callback(query: types.CallbackQuery, callback_data: DriverCallback, user: UserRes, bot) -> ReturnValue:
    ...
    elif callback_data.type == DriverCallbackType.SEND_ADMIN:
        return await send_admin(callback_data=callback_data, user=user.data(), bot=bot)
