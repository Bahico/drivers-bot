...
from admin.admin_callback import AdminCallback
...
async def send_message(chat_id: int, data: ReturnValue, message_id: int = None):
    if data:
        if data.reply_markup:
            ...
            if data.editMessage:
                await bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=data.message, parse_mode=ParseMode.HTML)
            else:
